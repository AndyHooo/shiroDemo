...
public class UserDaoImpl extends BasedDao implements UserDao {
	...
	public User queryByNameAndPass(String name, String pass) throws Exception {
		User user = null;
		Session session = getSession();
		String hql = "from User where name='"+name+"' and passwd='"+pass+"'";
		Query query = session.createQuery(hql);
		List list = query.list();
		if(list != null && list.size()>0){
			user = (User) list.get(0);
	...
	@Override
	public List<Per> getPermissionsByUserId(int id) throws Exception {
		Session session = getSession();
		
		String hql = "from Per where userId="+id;
		Query query = session.createQuery(hql);
		List list = query.list();
		return list;
